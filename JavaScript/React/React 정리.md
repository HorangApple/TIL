...
불변 데이터 객체를 포괄적으로 다루고자 하면, Immutable JS 같은 라이브러리를 사용한다.
...
현재 상태나 속성에 의존하는 상태를 갱신해야 하는 경우에는 prevState와 props 인수를 통해 기존의 상태를 알아낼 수 있다. Boolean 값을 토글할 때 갱신을 수행하기 앞서 정확한 기존의 상태를 알아야 하는 경우에 상당히 유용하다.
