...
class Collection extends React.Component {
  ...
  // 클릭 이벤트 처리, category의 idx를 받아 상태 변경
  _handleClick(i) {
    this.setState({
      activeIndex: i,
      open: true
  ...
  // category를 닫고 idx를 null로 초기화
  // category--image 클래스를 갖고있는 태그는 해당 없음
  _focusOff(e) {
    e.preventDefault();
  ...
  categoryNode(cat, i) {
    ...
    return (
      // categories를 map으로 불러들이면서 rendering
      <ProjectCategory
        cat={cat}
        key={"cat-" + i}
  ...
  render() {
    ...
    // classNames는 클래스를 토글역할 하게끔 함
    let classes = classNames({
      focused: this.state.open
    });
    return (
      // categories 5개를 감싸는 상위 div와 그 categories들
      // 바깥쪽을 클릭하면 _focusOff로 인해 category가 닫힘
      // window.innerHeight은 브라우저 윈도우 두께를 제외한 실질적 세로 길이
      <div
