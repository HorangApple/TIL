...
# 다음과 같이 작성하여 실행해도 같은 결과가 나온다.
# from a import cube, square 
# print(cube(2))
# print(square(2))
```

`b.py`를 실행 시키면 `a.py` 전체를 실행시키기 때문에 결과 출력이 2번씩 나오게 된다. 그렇기 때문에 `a.py`에 다음 조건문을 추가시키면 된다.
...
`__name__`는 모듈로 다른 곳에서 실행되면 그 파일명으로 초기화 되고 그 파일이 모듈이 아니고 직접 실행 되면 `__main__`이 초기화가 된다.

클래스를 선언할 때 대문자로 시작하는 이름으로 선언한다.

C, Java는 컴파일 언어로 컴퓨터 언어로 변형시키는 컴파일 작업을 하지만 파이썬은 스크립팅 언어로 그런 과정이 없다. 그렇기 때문에 파이썬은 C언어의 `main`함수처럼 무엇을 기준으로 먼저 실행해야할지가 처음부터 나와 있지 않다. 그렇기 때문에 `if __name__ == "__main__":`와 같은 조건문을 추가시키는 것이다.

```python
from flask import *
```

위와 같은 방식으로 모듈을 추가시키면 `flask`의 모든 함수를 사용할 수 있으나 잘못하면 함수 이름이 겹쳐버리는 문제가 발생할 수 있기때문에 사용할 함수만 import 시키는 것이 좋다.
...
- 클래스(Class) - 분류 체계. 같은 종류(또는 문제 해결을 위한)의 집단에 속하는 속성(attribute)과 행위(behavior)를 정의한 것으로 객체지향 프로그램의 기본적인 사용자 정의 데이터형(user define data type)이라고 할 수 있다. 클래스는 프로그래머가 아니지만 해결해야 할 문제가 속하는 영역에 종사하는 사람이라면 사용할 수 있고, 다른 클래스 또는 외부 요소와 독립적으로 디자인하여야 한다.
...
컴퓨터는 저장과 계산 밖에 못한다. 그렇기 때문에 프로그래밍 3형식은 저장, 조건, 반복으로 구성된다. 이를 만족하면 Turing complete (튜링 완전 언어)로 취급받는다.

세상을 요약하는 방법으로 철학적인 관점에서 주어와 동사, 주부와 (서)술부, Subject&Predicate, Object&Method이 있다. 이 중 Object&Method를 프로그래밍에서 사용한다. Object는 세상에 존재하는 모든 물체, 사물, Thing을 가리킨다. 이는 분류 체계 속의 예시라 할 수 있다. 예를 들면 나무 분류체계를 들 수 있다. Method는 행위를 가리킨다.

Object oriented programming은 세상 모든 것을 인간이 인식하는 형태로 프로그래밍 하는 방법, 즉 상식적인 프로그래밍이라 할 수 있다.

프로그래밍은 더이상 혼자서 작업하는 것이 아니기 때문에 OOP는 필수적인 요소로 자리잡을 수 있게 되었다. OOP의 아이디어를 만들어 낸 창시자 중 한 명은 엘런 케이이다.
...
class Person:                      #=> 클래스 정의(선언) : 클래스 객체 생성
    ...
    def __init__ (self):
        self.name='아이유'         #=> 인스턴스 변수(데이터 어트리뷰트) : 각각의 인스턴스(객체)가 독립적으로 가진 속성
    def greeting(self):            #=> 멤버 메서드(메서드)
        print(f'{self.name}')
```

클래스 변수와 인스턴스 변수를 합쳐서 멤버 변수라고도 한다.
...
- 굳이 'self'가 아닌 다른 말로 적어도 상관없는데 꼭 통일시켜야 하고 함수 선언에서 첫 인자는 무조건 self가 들어가는 위치라고 생각하면 된다.
